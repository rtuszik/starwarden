...
def get_existing_links(linkwarden_url, linkwarden_token, collection_id, delete_duplicate=False):
    ...
    Args:
        ...
        linkwarden_token: Authorization token for API
        collection_id: Collection ID to fetch links from
        delete_duplicate: If True, automatically delete duplicate links found
        
    Returns:
        Generator yielding all link URLs (both unique and duplicates)
        When delete_duplicate=True, still yields URLs while also handling deletion
    """
...
def delete_links(linkwarden_url, linkwarden_token, link_ids):
    """
    Bulk delete multiple links by their IDs.
    ...
    Returns:
        int: Number of successfully deleted links, or None if error occurred
    """
    url = f"{linkwarden_url.rstrip('/')}/api/v1/links"
    headers = {
        "Authorization": f"Bearer {linkwarden_token}",
