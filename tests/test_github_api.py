...
def test_starred_repos_rate_limit(github_star_manager):
    ...
    mock_starred.get_page.side_effect = [
        RateLimitExceededException(403, "API rate limit exceeded", headers={"Retry-After": "1"}),
    ...
    assert len(repos) == 1
    assert repos[0].html_url == "https://github.com/user/repo1"
    assert github_star_manager.handle_rate_limit.call_count == 1 if hasattr(github_star_manager.handle_rate_limit, 'call_count') else True
...
def test_handle_rate_limit(github_star_manager):
    ...
    with patch('time.sleep') as mock_sleep:
        github_star_manager.handle_rate_limit(Mock(headers={"Retry-After": "2"}), skip_sleep=True)
