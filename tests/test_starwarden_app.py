...
from starwarden import StarwardenApp, StarwardenError, configure_logging, GithubStarManager, LinkwardenManager, check_test_coverage
from github import GithubException

@pytest.fixture
def starwarden_app():
    with patch('starwarden.GithubStarManager'), patch('starwarden.LinkwardenManager'), \
...
def test_run(mock_print, starwarden_app):
    ...
    starwarden_app.linkwarden_manager.get_existing_links.return_value = ["https://github.com/repo1"]
...
def test_select_or_create_collection_new(mock_ask, starwarden_app, caplog):
    ...
    starwarden_app.linkwarden_manager.create_collection.assert_called_once_with("new_collection")
    ...
    starwarden_app.linkwarden_manager.create_collection.assert_called_once_with("another_new_collection")
...
def test_run_with_new_collection(mock_ask, mock_print, starwarden_app):
    ...
    with patch.object(starwarden_app, 'main_menu', return_value=2), \
         patch.object(starwarden_app, 'select_or_create_collection', return_value=1):
        ...
        mock_print.assert_called()
        mock_ask.assert_called_once_with("Enter the name for the new GitHub Stars collection")
        starwarden_app.linkwarden_manager.create_collection.assert_called_once_with("new_collection")
...
def test_run_with_request_exception(mock_exception, starwarden_app, caplog):
    ...
    assert any("Unexpected error processing user/repo1" in record.message
...
def test_linkwarden_manager_upload_link_failure(mock_post):
    ...
    mock_repo = Mock(html_url="https://github.com/test/repo", full_name="test/repo", description="Test repo")
...
def test_github_star_manager_initialization_failure(mock_github):
    ...
    assert "Failed to initialize GitHub user: 404 \"User not found\"" in str(excinfo.value)

def test_initialize_user_error_handling():
    with patch('starwarden.Github') as mock_github, \
         patch('starwarden.ArgumentParser.parse_args') as mock_parse_args:
...
@patch('starwarden.StarwardenApp.main_menu')
@patch('starwarden.StarwardenApp.select_or_create_collection')
def test_run_with_no_collection_selected(mock_select, mock_menu, mock_welcome, starwarden_app):
    starwarden_app.args.id = None
    mock_menu.return_value = 1
